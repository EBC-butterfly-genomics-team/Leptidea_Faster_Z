...
PATH_LOCAL=1c_QC_fastq_masker                                 #folder containing input data
...
OUT_FOLDER=1d_QC_filterPolyA_prinseq/              #output folder
...
for i in `seq 1 1 $N_SAMPLES`; do                  #loop starting all jobs
   ...
   echo $RNAFILE_B
   sbatch filterPolyA_prinseq_run_query.sh $PATH_MAIN/$PATH_LOCAL/$RNAFILE_A \
